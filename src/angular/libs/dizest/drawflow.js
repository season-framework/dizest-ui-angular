...
export default class Drawflow {
        ...
        switch (this.ele_selected.classList[0]) {
            case 'drawflow-node':
                ...
                if (this.connection_selected != null) {
                    this.connection_selected.parentElement.classList.remove("selected");
            ...
            case 'parent-drawflow':
                if (this.node_selected != null) {
                    ...
                    this.dispatch('nodeUnselected', false);
                }
        ...
        if (this.connection === true) {
            if (ele_last.classList[0] === 'input' || this.force_first_input && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node')) {
                if (this.force_first_input && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node')) {
                    ...
                    // Fix connection;
                    var input_id = ele_last.parentElement.parentElement.parentElement.parentElement.id;
                    var input_class = ele_last.classList[1];
                }

                var output_id = this.ele_selected.parentElement.parentElement.parentElement.parentElement.id;
                ...
                if (output_id !== input_id && input_class !== false) {
                    if (this.container.querySelectorAll('.connection.node_in_' + input_id + '.node_out_' + output_id + '.' + output_class + '.' + input_class).length === 0) {
                        ...
                        // allow single connection for output
                        // if (this.drawflow.drawflow[this.module].data[id_input].inputs[input_class].connections.length > 0) {
                        //     return;
                        // }
    ...
    addNode(newNodeId, name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false) {
        if (!newNodeId) {
            if (this.useuuid) {
        ...
        let create_obj = (clsname, text) => {
            const container = document.createElement('div');
            clsname = clsname.split(' ');
            for (let i = 0; i < clsname.length; i++) {
                let _clsname = clsname[i];
                container.classList.add(_clsname);
            }
        ...
        input_container.appendChild(inputs);
        for (var x = 0; x < num_in.length; x++)
            input_container.appendChild(create_obj("input-text", num_in[x].substring(6)));
        if (num_out.length > num_in.length)
            input_container.appendChild(create_obj("input-text disabled", ' '));
        variable_container.appendChild(input_container);

        const output_container = create_obj("output-container");
        output_container.appendChild(outputs);
        for (var x = 0; x < num_out.length; x++)
    ...
    removeConnection() {
        if (this.connection_selected != null) {
            ...
            this.connection_selected.parentElement.remove(); //console.log(listclass);
